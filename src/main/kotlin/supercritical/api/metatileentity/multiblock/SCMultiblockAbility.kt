package supercritical.api.metatileentity.multiblock

import com.gregtechceu.gtceu.api.machine.multiblock.PartAbility

object SCMultiblockAbility {
    val IMPORT_FUEL_ROD: PartAbility = PartAbility("import_fuel_rod")
    val EXPORT_FUEL_ROD: PartAbility = PartAbility("export_fuel_rod")
    val IMPORT_COOLANT: PartAbility = PartAbility("import_coolant")
    val EXPORT_COOLANT: PartAbility = PartAbility("export_coolant")
    val CONTROL_ROD_PORT: PartAbility = PartAbility("control_rod_port")
    val MODERATOR_PORT: PartAbility = PartAbility("moderator_port")
}
