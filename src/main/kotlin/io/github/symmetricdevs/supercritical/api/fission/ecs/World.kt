...
class World(val schedule: Schedule) {
    ...
    private fun componentTypeOf(component: Component): ComponentType<*> {
        val type = ComponentTypeRegistry.byClass(component::class)
        requireNotNull(type) {
            "No ComponentType is registered for ${component::class.qualifiedName}. " +
                "Ensure ReactorComponentTypes.registerAll(ComponentTypeRegistry) runs before using addComponent."
        }
        return type
    }
