...
// [io.github.symmetricdevs.supercritical.api.fission.reactor.pwr.PWRSchedule].
// ----------------------------------------------------------------------------------
...
class ThermalHydraulicsSystem : System {
    ...
    private fun makeCoolantFlow(world: World): Double {
        ...
        for (entity in cache.coolantChannels) {
            ...
            val hotCoolant = requireNotNull(prop.hotCoolantFluid) { "Coolant must define a hot coolant fluid for coolant flow" }
...
/**
 * Xenon/iodine poisoning evolution, ported from `PWRCore.updateNeutronPoisoning`.
 */
...
/**
 * Automatic control-rod regulation, ported from `PWRCore.regulateControlRods`.
