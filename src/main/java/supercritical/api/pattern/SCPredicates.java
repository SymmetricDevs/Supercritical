        ...
        return new TraceabilityPredicate(bws -> {
            ...
            if (bws.getWorld().isAirBlock(bws.getPos()) || blockState.getBlock() == fluidBlock) {
                bws.getMatchContext()
                        ...
                        .getOrPut(FLUID_BLOCKS_KEY, new ArrayList<>()).add(bws.getPos());
                return true;
            }
            return false;
        }, () -> new BlockInfo[] { SCConfigHolder.misc.showFluidsForAutoFillingMultiblocks ? new BlockInfo(stillState) :
                new BlockInfo(Blocks.AIR) });
    }
