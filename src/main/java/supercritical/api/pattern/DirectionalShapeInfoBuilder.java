...
/**
 * Extends {@link MultiblockShapeInfo.Builder} to incorporate directional
 * awareness.
 */
public class DirectionalShapeInfoBuilder extends MultiblockShapeInfo.Builder {
	...
	/**
	 * @param structureDir
	 *            The directions that the provided block pattern is based upon
	 *            (character, string, row).
	 */
	...
	/**
	 * @param partSupplier
	 *            Should supply either a MetaTileEntity or an IBlockState.
	 */
	@Override
	...
	@NotNull private BlockInfo[][][] bakeArray() {
		final int maxZ = self.getShape().size();
		final int maxY = self.getShape().get(0).length;
		final int maxX = self.getShape().get(0)[0].length();

		BlockPos end = setActualRelativeOffset(maxX, maxY, maxZ, EnumFacing.SOUTH, EnumFacing.UP, true, structureDir);
		...
		for (int z = 0; z < maxZ; z++) {
			...
			for (int y = 0; y < maxY; y++) {
				...
				for (int x = 0; x < maxX; x++) {
					...
					BlockPos pos = setActualRelativeOffset(x, y, z, EnumFacing.SOUTH, EnumFacing.UP, true, structureDir)
							.add(addition);
	...
	public static BlockPos setActualRelativeOffset(int x, int y, int z, EnumFacing facing, EnumFacing upwardsFacing,
		...
		if (facing == EnumFacing.UP || facing == EnumFacing.DOWN) {
			...
			if (upwardsFacing == EnumFacing.WEST || upwardsFacing == EnumFacing.EAST) {
				int xOffset = upwardsFacing == EnumFacing.WEST
						? facing.rotateY().getXOffset()
						: facing.rotateY().getOpposite().getXOffset();
				int zOffset = upwardsFacing == EnumFacing.WEST
						? facing.rotateY().getZOffset()
