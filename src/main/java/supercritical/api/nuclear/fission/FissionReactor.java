...
public class FissionReactor {
	...
	/**
	 * The gas constant in J * K^-1 * mol^-1 if you want to use a different set of
	 * units prepare thy life to become the worst of nightmares
	 */
	...
	/**
	 * Temperature of boiling point in kelvin at standard pressure Determined by a
	 * weighted sum of the individual coolant boiling points in
	...
	/**
	 * Latent heat of vaporization in J/mol Determined by a weighted sum of the
	 * individual heats of vaporization in
	 * {@link FissionReactor#prepareInitialConditions()}
	 */
	private double coolantHeatOfVaporization;
	/**
	 * Equilibrium temperature in kelvin Determined by a weighted sum of the
	 * individual coolant temperatures in
	 * {@link FissionReactor#prepareInitialConditions()}
	 */
	...
	// 0.06m is ideal for a 1-m diameter steel pipe with the given maximum, and was
	// the original value
	...
	protected double responseFunctionTemperature(double envTemperature, double currentTemperature, double heatAdded,
		...
		/*
		 * Simplifies what is the following: heatTransferCoefficient = 1 / (1 /
		 * convectiveHeatTransferCoefficient + wallThickness / thermalConductivity);
		 * (https://en.wikipedia.org/wiki/Newton%27s_law_of_cooling#First-
		 * order_transient_response_of_lumped- capacitance_objects) This assumes that
		 * we're extracting heat from the reactor through the wall into slowly moving
		 * air, removing the second convective heat. timeConstant =
		 * heatTransferCoefficient * this.surfaceArea / specificHeatCapacity;
		 */
		...
		double effectiveEnvTemperature = envTemperature + (heatAdded - heatAbsorbed)
				/ (timeConstant * (this.coolantMass + this.structuralMass + this.fuelMass));
	...
	public void prepareThermalProperties() {
		structuralMass = reactorDepth * reactorRadius * reactorRadius * Math.PI * 300; // Assuming 300 kg/m^3 when it's
																						// basically empty, does not
																						// have to be precise
		...
		for (int i = 0; i < reactorLayout.length; i++) {
			for (int j = 0; j < reactorLayout[i].length; j++) {
				/*
				 * Check for null because the layout is in general not a square
				 */
	...
	public double computeK(boolean addToEffectiveLists, boolean controlRodsInserted) {
		...
		/*
		 * We calculate geometric factor matrices to determine how many neutrons go from
		 * the i-th to the j-th fuel rod This factor is different for slow and fast
		 * neutrons because they interact differently with the materials and fuel
		 */
		for (int i = 0; i < fuelRods.size(); i++) {
			for (int j = 0; j < i; j++) {
				double mij = 0; // Integrates over the moderation factor; read "moderation from I to J"
				double saij = 0; // Integrates over slow neutron fission, scattering and capture; read "slow
									// absorption
				// from I to J"
				double faij = 0; // Integrates over fast neutron fission, scattering and capture; read "fast
									// absorption
				// from I to J"
				...
				/*
				 * Geometric factor calculation is done by (rough) numerical integration along a
				 * straight path between the two cells
				 */
				...
				for (int t = 0; t < resolution; t++) {
					...
					if (!component.samePositionAs(fuelRods.get(i)) && !component.samePositionAs(fuelRods.get(j))) {
						saij += component.getAbsorptionFactor(controlRodsInserted, true);
						faij += component.getAbsorptionFactor(controlRodsInserted, false);
					}

					// Doing this second so that water's absorption is not overbearing compared to
					// its moderation.
					...
					/*
					 * We keep track of which active elements we hit, so we can determine how
					 * important they are relative to the others later
					 */
				...
				/*
				 * The actual calculation of the geometric factors, fast neutrons are randomly
				 * converted into slow neutrons along the path, we pretend that fuel rods are
				 * infinitely tall and thin for simplicity This means the fraction of slow
				 * neutrons will go as (1-exp(-m * x))/x where x is the distance between the
				 * cells The fraction of fast neutrons is simply one minus the fraction of slow
		...
		/*
		 * We now perform the power iteration algorithm to approximate kEff. We create a
		 * guess of a vector with all elements set to 1, which we then normalize. We
		 * then calculate the resulting vector by multiplying it with the geometric
		 * matrix, and normalize it again. This repeats a few times, and the resulting
		 * magnitude of the vector is the approximate kEff (the largest eigenvalue).
		 */
	...
	/**
	 * Loops over all the coolant channels and determine which ones are adjacent to
	 * fuel rods
	 */
	...
	/**
	 * Loops over all the control rods, determines which ones actually affect
	 * reactivity, and gives them a weight depending on how many fuel rods they
	 * affect
	 */
	...
	/**
	 * Consumes the coolant. Calculates the heat removed by the coolant based on an
	 * amalgamation of different equations. It is not particularly realistic, but
	 * allows for some fine-tuning to happen. Heat removed is proportional to the
	 * surface area of the coolant channel (which is equivalent to the reactor's
	 * depth), as well as the flow rate of coolant and the difference in temperature
	 * between the reactor and the coolant
	 */
		...
		for (CoolantChannel channel : coolantChannels) {
			...
			if (tryFluidDrain != null) {
				...
				double heatRemovedPerLiter = prop.getSpecificHeatCapacity()
						/ SCConfigHolder.nuclear.fissionCoolantDivisor * (cooledTemperature - coolantTemp);
				// Explained by:
				...
				int actualFlowRate = Math.min(remainingSpace, (int) (cappedFluidUsed + channel.partialCoolant));
				// Should occasionally decrease when coolant is actually consumed.
				channel.partialCoolant += cappedFluidUsed - actualFlowRate;

				FluidStack HPCoolant = new FluidStack(prop.getHotCoolant(), actualFlowRate);

				channel.getInputHandler().getFluidTank().drain(actualFlowRate, true);
				channel.getOutputHandler().getFluidTank().fill(HPCoolant, true);
				if (prop.accumulatesHydrogen() && this.temperature > zircaloyHydrogenReactionTemperature) {
	...
	/**
	 * Finds the maximum power that could be generated by the reactor with the
	 * current coolant, capped at either the max temperature or the zircaloy
	 * hydrogen reaction temperature.
	 */
	public double calculateMaxPower() {
		...
		for (CoolantChannel channel : coolantChannels) {
			...
			double heatRemovedPerLiter = prop.getSpecificHeatCapacity() / SCConfigHolder.nuclear.fissionCoolantDivisor
		...
		double timeConstant = specificHeatCapacity
				* (1 / convectiveHeatTransferCoefficient + wallThickness / thermalConductivity) / this.surfaceArea;
		/*
		 * Solving for x below: hypotheticalTemperature = envTemperature + (x -
		 * heatRemoved) / (timeConstant * (this.coolantMass + this.structuralMass +
		 * this.fuelMass));
		 */
		return ((hypotheticalTemperature - envTemperature)
				* (timeConstant * (this.coolantMass + this.structuralMass + this.fuelMass)) + heatRemoved) / 1e6;
	}

	/**
	 * Unrealistic in terms of the pressure effects, but it allows for a consistent
	 * power output.
	 */
	protected double coolantBoilingPoint() {
		...
		// this.coolantHeatOfVaporization);
	}
	...
	protected double coolantBoilingPoint(ICoolantStats coolant) {
		...
		/*
		 * return 1. / (1. / coolant.getBoilingPoint() - R * Math.log(this.pressure /
		 * standardPressure) / coolant.getHeatOfVaporization());
		 */
	}
	...
	public void updateTemperature() {
		...
		// prevent temperature from going above meltdown temp, to stop coolant from
		// absorbing more heat than it should
		this.temperature = Math.min(maxTemperature, temperature);
		double heatRemoved = this.makeCoolantFlow();
		// calculate the actual temperature based on the reactor power and the heat
		// removed
	...
	public void updatePressure() {
		this.pressure = responseFunction(!(this.temperature <= this.coolantBoilingPoint()) && this.isOn
				? 1000. * R * this.temperature
				: this.exteriorPressure, this.pressure, 0.2);
	}
	...
	public void updatePower() {
		if (this.isOn) {
			...
			// Since the power defect is a change in the reactivity rho (1 - 1 / kEff), we
			// have to do this thing.
			// (1 - 1 / k) = rho(k) => rho^-1(rho) = 1 / (1 - rho)
			// rho^-1(rho(k) - defect) is thus 1 / (1 - (1 - 1/k - defect)) = 1 / (1/k +
			// defect)
