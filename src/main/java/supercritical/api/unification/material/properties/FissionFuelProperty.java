...
public class FissionFuelProperty implements IMaterialProperty, IFissionFuelStats {
	...
	// How likely it is for a not-yet-moderated neutron to cause fission in this
	// fuel.
	@Getter
	private double fastNeutronFissionCrossSection;
	// The average time for a neutron to be emitted during a fission event. Do not
	// make this accurate.
	@Getter
	...
	public static FissionFuelPropertyBuilder builder(String id, int maxTemperature, int duration,
		...
		return new FissionFuelPropertyBuilder().id(id).maxTemperature(maxTemperature).duration(duration)
