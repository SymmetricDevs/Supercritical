...
@ExtensionMethod({OrePrefixExtension.Handler.class, MaterialExtension.Handler.class, ArmorLogicExtension.Handler.class})
public abstract class MixinMetaPrefixItem extends StandardMetaItem {
	...
	@Unique private void sc$handleHeatDamage(@NotNull Material material, @NotNull EntityLivingBase entity) {
	...
	@Unique private void sc$handleRadiationDamage(@NotNull Material material, EntityLivingBase entity) {
		double radiationDamage = prefix.getRadiationDamageFunction().apply(material.getDecaysPerSecond());
		ItemStack armor = entity.getItemStackFromSlot(EntityEquipmentSlot.CHEST);
		if (!armor.isEmpty() && armor.getItem() instanceof ArmorMetaItem<?>) {
			ArmorMetaItem<?>.ArmorMetaValueItem metaValueItem = ((ArmorMetaItem<?>) armor.getItem()).getItem(armor);
			if (metaValueItem != null) {
	...
	/**
	 * @author Tian_mi
	 * @reason Can't really figure out a better way than a total overwrite like
	 *         this...
	 */
	@Overwrite
	@Override
