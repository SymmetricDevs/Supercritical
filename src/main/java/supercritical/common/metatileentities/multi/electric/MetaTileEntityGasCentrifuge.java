...
public class MetaTileEntityGasCentrifuge extends RecipeMapMultiblockController {
    ...
    protected BlockPattern createStructurePattern() {
        return FactoryBlockPattern.start(FRONT, UP, RIGHT).aisle("SI", "HH", "CC", "CC", "CC", "CC", "CC")
                .aisle("EE", "HH", "CC", "CC", "CC", "CC", "CC").setRepeatable(1, 14)
                .aisle("OO", "HH", "CC", "CC", "CC", "CC", "CC").where('S', selfPredicate())
                .where('P', states(getPipeState())).where('H', states(getHeaterState()))
    ...
    private static IBlockState getHeaterState() {
        return SCMetaBlocks.NUCLEAR_CASING.getState(BlockNuclearCasing.NuclearCasingType.GAS_CENTRIFUGE_HEATER);
    }
