...
public class MetaTileEntityCoolantImportHatch extends MetaTileEntityMultiblockNotifiablePart
                                              implements
                                              IMultiblockAbilityPart<ICoolantHandler>,
                                              ICoolantHandler,
                                              IControllable,
    ...
    public ModularUI.Builder createTankUI(IFluidTank fluidTank, String title, EntityPlayer entityPlayer) {
        ...
        TankWidget tankWidget = new TankWidget(fluidTank, 119, 52, 18, 18).setHideTooltip(true).setAlwaysShowFull(true);
        ...
        return builder.label(6, 6, title)
                ...
                .widget(new SlotWidget(exportItems, 0, 140, 53, true, false).setBackgroundTexture(GuiTextures.SLOT,
                        GuiTextures.OUT_SLOT_OVERLAY))
    ...
    public MetaTileEntityCoolantExportHatch getExportHatch(int depth) {
        ...
        for (int i = 1; i < depth; i++) {
            if (getWorld().getBlockState(pos.move(this.frontFacing.getOpposite())) != SCMetaBlocks.FISSION_CASING
                    .getState(BlockFissionCasing.FissionCasingType.COOLANT_CHANNEL)) {
                return null;
            }
        }
