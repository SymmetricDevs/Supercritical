	...
	public static void init() {
		...
		Uraninite.setProperty(SCPropertyKey.FISSION_FUEL,
				FissionFuelProperty.builder(Uraninite.getRegistryName(), 1800, 60000, 2.4)
						.fastNeutronCaptureCrossSection(0.5).slowNeutronCaptureCrossSection(1)
						.slowNeutronFissionCrossSection(1).requiredNeutrons(1).releasedNeutrons(2.5)
						.releasedHeatEnergy(0.01).decayRate(0.001).build());
		...
		DistilledWater.setProperty(SCPropertyKey.COOLANT,
				new CoolantProperty(DistilledWater, HighPressureSteam, FluidStorageKeys.LIQUID, 2., 1000, 373, 2260000,
						4168.).setAccumulatesHydrogen(true).setSlowAbsorptionFactor(0.1875)
						.setFastAbsorptionFactor(0.0625));

		Graphite.setProperty(SCPropertyKey.MODERATOR,
				ModeratorProperty.builder().maxTemperature(3650).absorptionFactor(0.0625).moderationFactor(3).build());
		Graphite.addFlags(MaterialFlags.FORCE_GENERATE_BLOCK);

		Beryllium.setProperty(SCPropertyKey.MODERATOR, ModeratorProperty.builder().maxTemperature(1500)
				.absorptionFactor(0.015625).moderationFactor(5).build());
